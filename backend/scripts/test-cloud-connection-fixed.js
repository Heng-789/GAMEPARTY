/**
 * Test Cloud PostgreSQL Connection (Fixed SSL)
...
function createPoolConfig(connectionString) {
  ...
  // Force SSL config for Supabase - MUST set rejectUnauthorized: false
  if (useSSL) {
    config.ssl = {
...
if (process.env.DATABASE_URL_HENG36) {
  const config = createPoolConfig(process.env.DATABASE_URL_HENG36);
  console.log('HENG36 Config:', JSON.stringify(config, null, 2));
  pools.heng36 = new Pool(config);
}

if (process.env.DATABASE_URL_MAX56) {
  const config = createPoolConfig(process.env.DATABASE_URL_MAX56);
  pools.max56 = new Pool(config);
}

if (process.env.DATABASE_URL_JEED24) {
  const config = createPoolConfig(process.env.DATABASE_URL_JEED24);
  pools.jeed24 = new Pool(config);
}

// Fallback to single DATABASE_URL
if (Object.keys(pools).length === 0 && process.env.DATABASE_URL) {
  const config = createPoolConfig(process.env.DATABASE_URL);
  pools.default = new Pool(config);
}
...
async function testConnection() {
  ...
  try {
    ...
    console.error('6. Check if project is active in Supabase Dashboard');
    process.exit(1);
  } finally {
    // Close all pools
